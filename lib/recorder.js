...
export function createFFmpeg(displayConfig, filepath, errorCallback) {
  ...
  // Add screen input parameters
  if (displayConfig.screenFormat === 'gdigrab') {
    // Windows gdigrab needs specific parameters
    args.push(
      '-framerate', '30',
  ...
  // Add audio input - Windows (dshow) needs special formatting
  if (displayConfig.audioFormat === 'dshow' && displayConfig.audioDevice) {
    args.push('-f', displayConfig.audioFormat);
    args.push('-i', displayConfig.audioDevice);
  } else if (displayConfig.audioInput && displayConfig.audioFormat) {
    args.push('-f', displayConfig.audioFormat);
    args.push('-i', displayConfig.audioInput);
  }

  // Output encoding settings - use compatible codecs
  let videoCodec = 'libx264';
  let audioCodec = 'aac';

  // Windows fallback to mpeg4 if libx264 fails
  if (platform() === 'win32') {
    videoCodec = 'mpeg4';
  ...
  if (displayConfig.audioInput || displayConfig.audioDevice) {
    args.push('-c:a', audioCodec, '-b:a', '192k');
  }
